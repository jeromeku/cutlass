...
prof = torch.profiler.profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], with_stack=True)
...
for size in [2 ** i for i in range(8, 12)]:
    ...
    print(prof.key_averages().table(sort_by="cuda_time_total", max_name_column_width=100, header=f"Matmul {M}x{N}x{K}"))
# %%
# Benchmark
# ---------
#
