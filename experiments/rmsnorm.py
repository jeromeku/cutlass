...
patch_cutlass_env(dumpdir="rms_norm_dump", keep_ptx=True, keep_ir=True, lineinfo=True, arch="sm_100a")
...
class CtaNorm:
    def __init__(
        ...
        self.dtype = dtype
        self.elems_per_thread = UNIVERSAL_COPY_BITS // dtype.width
        self.warp_size = 32
    ...
    def kernel(
        ...
        print(f"[DSL INFO]   gX pre slice = {gX}")
        print(f"[DSL INFO]   gY pre slice = {gY}")

        gY, gX = gY[0, None], gX[0, None]
        print(f"[DSL INFO]  {tiled_copy}")
        print(f"[DSL INFO]  {thr_copy}")

        print("[DSL INFO] Tiled Tensors:")
        print(f"[DSL INFO]   gX post slice = {gX}")
        print(f"[DSL INFO]   gY post slice = {gY}")
        ...
        cute.print_tensor(pred, verbose=True)
        print(f"[DSL INFO]   pred = {pred}")
        print(f"[DSL INFO]   tYgY = {tYgY}")
        print(f"[DSL INFO]   tXgX = {tXgX}")
        print(f"[DSL INFO]   tWgW = {tWgW}")
        print(f"[DSL INFO]   mWeight = {mWeight}")
        ...
        print(f"[DSL INFO]   tXgX = {tXgX}")
        print(f"[DSL INFO]   tWgW = {tWgW}")
        print(f"[DSL INFO]   tXrX = {tXrX}")
        print(f"[DSL INFO]   tWrW = {tWrW}")
        if cutlass.const_expr(self.norm_type == "layer"):
            print(f"[DSL INFO]   tBgB = {tBgB}")
    ...
    def apply_layernorm(
        ...
        print(f"Layernorm: x[None, 0] {x[None, i]}")
...
def run_layernorm(
    ...
    ALIGNMENT = 8 * (dtype.width // 8)
    x_fake = cute.runtime._FakeTensor(dtype=dtype, shape=tuple(x.shape), stride=tuple(x.stride()), assumed_align=ALIGNMENT)
    ...
    options = None
    # options = (GenerateLineInfo(True), DumpDir("./cute_layernorm"), KeepPTX(True))
    # options = CompileOptions(options)
    # print(f"CompileOptions:\n{options.to_str()}")
    from cutlass.cutlass_dsl.cuda_jit_executor import CudaDialectJitCompiledFunction
    from cutlass.cutlass_dsl import CuTeDSL
    from utils.cute_mlir_pipeline_override import enable_pipeline_tracing
    # enable_pipeline_tracing(dump_dir="./rms_norm_mlir_dump")
    dsl = CuTeDSL._get_dsl()

    if norm_type == "layer":
        compiled_func: CudaDialectJitCompiledFunction = compiler(
            layernorm, _y, _x, _weight, _bias, options=options)
    else:
        compiled_func: CudaDialectJitCompiledFunction = compiler(
            layernorm, _y, _x, _weight, _bias, options=options,
        )
    ...
    compile_opts = dsl.compile_options
    # print(f"Compile options: {compile_opts.to_str()}")
    # print(f"GPU Arch: {compile_opts.gpu_arch}")
    # print(f"Lineinfo: {compile_opts.generate_line_info}")
    # print(f"PTX Path: {compile_opts.full_ptx_path}")
