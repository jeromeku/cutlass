...
def patch_cutlass_env(
    ...
    keep_ir: bool = False,
    keep_ptx: bool = False,
    keep_cubin: bool = False,
    lineinfo: bool = True,
    optimization_warnings: bool = True,
    print_after_preprocessor: bool = False,
    preprocessed_ast_path: str = None
):
    # Need to set env flags before importing cutlass
    if arch is not None:
    ...
    if keep_ir:
        os.environ[f"{PREFIX}_KEEP_IR"] = "1"
    if dumpdir:
        os.environ[f"{PREFIX}_DUMP_DIR"] = dumpdir
        os.makedirs(dumpdir, exist_ok=True)
    if keep_ptx:
        os.environ[f"{PREFIX}_KEEP_PTX"] = "1"
    if keep_cubin:
        os.environ[f"{PREFIX}_KEEP_CUBIN"] = "1"
    if lineinfo:
        os.environ[f"{PREFIX}_LINEINFO"] = "1"
    if optimization_warnings:
        os.environ[f"{PREFIX}_OPTIMIZATION_WARNINGS"] = "1"
    if print_after_preprocessor:
        os.environ[f"{PREFIX}_PRINT_AFTER_PREPROCESSOR"] = "1"
        if preprocessed_ast_path is not None:
            os.environ[f"{PREFIX}_AST_PREPROCESSED_PATH"] = preprocessed_ast_path
