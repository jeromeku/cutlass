...
def get_joint_graph(fn, *example_inputs):
    primals = [
    ...
    def joint(*args):
        ...
        fwd = fn(*p)
        fwd_t = fwd if isinstance(fwd, tuple) else (fwd,)
        diff_p = [x for x in p if isinstance(x, torch.Tensor) and x.requires_grad]
        grads = torch.autograd.grad(fwd_t, diff_p, grad_outputs=t,
                                    allow_unused=True, create_graph=False)
        return (*fwd_t, *grads)

    gm = make_fx(joint)(*primals, *tangents)
    return gm, primals, tangents


# ── partition ─────────────────────────────────────────────────────────────────

def partition(fn, *example_inputs, use_min_cut=False):
    """
    ...
    """
    joint_gm, primals, tangents = get_joint_graph(fn, *example_inputs)
...
if __name__ == "__main__":
    ...
    for label, use_min_cut in [("default_partition", False),
        ...
        fw, bw = partition(two_layer, x, W1, b1, W2, target,
    ...
    fw, bw = partition(two_layer, x, W1, b1, W2, target, use_min_cut=True)

    tangent = torch.ones(())

    # fw takes ALL joint inputs (primals + tangents) and returns (loss, *saved)
    fw_outs   = fw(x, W1, b1, W2, target, tangent)
    loss_sym  = fw_outs[0]
    saved     = fw_outs[1:]
    ...
    loss_ref = two_layer(xr, W1r, b1r, W2r, target)
    loss_ref.backward()
