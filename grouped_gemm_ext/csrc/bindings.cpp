...
at::Tensor create_grouped_gemm_output_tensor(const at::Tensor &mat_a,
                                             ...
                                             std::optional<c10::ScalarType> out_dtype)
{
  ...
  const auto out_dtype_ = out_dtype.value_or(c10::kBFloat16);
  TORCH_CHECK(out_dtype_ == c10::kBFloat16, "Only bf16 high precision output types are supported for grouped gemm");
...
bool check_valid_strides_and_return_transposed(const at::Tensor &mat)
{
    ...
    std::optional<c10::ScalarType> out_dtype)
{
  ...
  at::Tensor out = create_grouped_gemm_output_tensor(mat_a, mat_b, offs, out_dtype);
