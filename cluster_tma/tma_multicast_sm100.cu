            ...
            Alpha alpha, Beta beta)
{
  ...
  dim3 cluster_id = cute::block_id_in_cluster();
  ...
  int bdimx = gridDim.x;
  int bdimy = gridDim.y;
  bool should_print = threadIdx.x == 0 && threadIdx.y == 0 && cluster_rank == 3;
  ...
  if (should_print) {
    printf("bdim (%d, %d), bid: (%d, %d)\n", blockDim.x, blockDim.y, blockIdx.x, blockIdx.y);
    printf("cluster size (%d, %d), cluster rank: %d, cluster id: (%d, %d)\n", cluster_dims.x, cluster_dims.y, cluster_rank, cluster_id.x, cluster_id.y);
                                   ...
                                   Alpha const alpha, Beta const beta)
{
  ...
  auto cluster_shape = make_shape(Int<cluster_m>{}, Int<cluster_n>{}, Int<1>{});
  Layout cluster_layout_vmnk = tiled_divide(make_layout(cluster_shape),
